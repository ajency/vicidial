...
function facetCategoryChange(thisObj) {
  ...
  if (search_string == true) {
    append_filter_str = url.indexOf("?") == url.length - 1 ? "" : url.indexOf("?") !== -1 ? "&" : "?";
    if (search_string_filter.trim() != "") url += append_filter_str + "search_string=" + search_string_filter;
  }
    ...
    if (is_ajax == true) {
      ...
      $.ajax({
        ...
        if (isMobile == false) {
          ...
          } else window.history.pushState('categoryPage', 'Category', url);
        }
        if (is_ajax == true && isMobile == true) {
          ...
          if ($('.nav-item.active').find('.filter-count').hasClass('d-none')) var filter_count = 0;else var filter_count = parseInt($('.nav-item.active').find('.filter-count').text());

          if ($(thisObj).prop('checked')) filter_count += 1;else filter_count -= 1;
          if (filter_count < 0) filter_count = 0;
          $('.nav-item.active').find('.filter-count').text(filter_count);
          if (filter_count == 0) $('.nav-item.active').find('.filter-count').addClass('d-none');else $('.nav-item.active').find('.filter-count').removeClass('d-none');
        }
        // if(search_string == true)
