    ...
    function addToCart() {
        ...
        $.ajax({
            ...
            error: function error(request, status, _error) {
                ...
                if (request.responseJSON && request.responseJSON.status == 401) userLogout();else if (!isLoggedInUser() || request.status == 0) showErrorPopup(request);else {
                    if (request.responseJSON.status == 400 || request.responseJSON.status == 403) getNewCartId();else showErrorPopup(request);
                }
            }
        });
    }
...
function loadAngularApp() {
    if (!loaded) {
        $.when($.getScript("/views/cart/inline.bundle.js"), $.getScript("/views/cart/polyfills.bundle.js"), $.getScript("/views/cart/styles.bundle.css"), $.getScript("/views/cart/vendor.bundle.js"), $.Deferred(function (deferred) {
            $(deferred.resolve);
        })).done(function () {
