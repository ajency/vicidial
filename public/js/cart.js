...
$(document).ready(function () {
    ...
    if (window.location.href.endsWith('#bag') || window.location.href.endsWith('#bag/user-verification') || window.location.href.endsWith('#shipping-address') || window.location.href.endsWith('#shipping-summary')) openCart();
    ...
    function openAccountAppOnUrlChange() {
        if (!$('#cd-my-account').hasClass("speed-in") && (window.location.href.endsWith('#/account/my-orders') || window.location.href.endsWith('#/account') || window.location.href.endsWith('#/account/user-verification'))) openMyAccountPage();
    }
    ...
    $('.cd-add-to-cart').on('click', function () {
        ...
        if ($('input[type=radio][name=kss-sizes]:checked').length == 0) {
            ...
            setTimeout(function () {
                jQuery(".kss_sizes").removeClass("shake");
            }, 200);
        } else {
            addToCart();
        }
    ...
    function addToCart() {
        ...
        $.ajax({
            ...
            error: function error(request, status, _error) {
                ...
                if (request.status == 401) userLogout(request);else if (request.status == 0) showErrorPopup(request);else if (!isLoggedInUser() && request.status == 403) {
                    addToCart();
                } else {
                    if (isLoggedInUser() && request.status == 400 || request.status == 403) getNewCartId(request);else showErrorPopup(request);
                }
            }
        });
    }
...
function loadAngularApp() {
    if (!loaded) {
        $.getScript("/views/cart/inline.bundle.js").done(function (script, textStatus) {
            // console.log(textStatus);
            $.getScript("/views/cart/vendor.bundle.js").done(function (script2, textStatus2) {
                // console.log(textStatus2);
                $.getScript("/views/cart/polyfills.bundle.js").done(function (script3, textStatus3) {
                    // console.log(textStatus3);
                    $.getScript("/views/cart/main.bundle.js").done(function (script4, textStatus4) {
...
function isLoggedInUser() {
    if (getCookie('token') && getCookie('cart_id')) return true;
    return false;
}
...
function fbTrackAddToCart(var_id) {
    var variant = variants[selected_color_id].variants.find(function (variant) {
        return variant.id == var_id;
    });
    fbq('track', 'AddToCart', {
...
function loadMyAccountApp() {
    $.getScript("/views/my-account/inline.bundle.js").done(function (script, textStatus) {
        // console.log(textStatus);
        $.getScript("/views/my-account/vendor.bundle.js").done(function (script2, textStatus2) {
            // console.log(textStatus2);
            $.getScript("/views/my-account/polyfills.bundle.js").done(function (script3, textStatus3) {
                // console.log(textStatus3);
                $.getScript("/views/my-account/main.bundle.js").done(function (script4, textStatus4) {
                    // console.log(textStatus4);
                }).fail(function (jqxhr, settings, exception) {
                    // console.log("angular load failed")
                    // loadAngularApp();
