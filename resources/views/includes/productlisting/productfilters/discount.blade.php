<script id="filter-discount-template" type="text/x-handlebars-template">
  <div class="kss_filter-list mt-1" data-filter="@{{template}}">
      <div class="filter-heading">
        <label class=" w-100 mb-0 pb-3 cursor-pointer @{{#if collapsed}} collapsed @{{/if}}" data-toggle="collapse" data-target="#collapseDiscount" aria-expanded="false" aria-controls="collapseDiscount">
         @{{filter_display_name}} <i class="fas fa-angle-up float-right"></i>
        </label>
      </div>
      <div id="collapseDiscount" class="collapse@{{#if collapsed}}@{{else}} show @{{/if}}" data-field="@{{template}}">
...
<div id="filter-discount-template-content"></div>
@section('footjs-discount')
<script type="text/javascript" >
   // require('handlebars');
   var source   = document.getElementById("filter-discount-template").innerHTML;
   ...
   document.getElementById("filter-discount-template-content").innerHTML = html;
 </script>
 @stop