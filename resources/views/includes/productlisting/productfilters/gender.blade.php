      ...
      <div id="collapseGender" class="collapse @{{#if collapsed}} @{{else}} show @{{/if}}" aria-labelledby="headingOne" data-parent="#accordion">
        <div class="card-body">
          ...
          <div class="custom-radio custom-control">
            <input type="radio" class="facet-category custom-control-input" name="gender" value="@{{facet_value}}" @{{#if is_selected }} checked = "checked" @{{/if}} data-facet-name="@{{../filter_facet_name}}" data-singleton="true" data-slug="@{{slug}}" @{{#ifEquals count 0 }} disabled = "disabled" @{{/ifEquals}}>
          ...
          <div class="custom-control custom-checkbox" >
            <input type="checkbox" class="facet-category custom-control-input" name="gender" value="@{{facet_value}}" @{{#if is_selected }} checked = "checked" @{{/if}} data-facet-name="@{{../filter_facet_name}}" data-singleton="false" data-slug="@{{slug}}" @{{#ifEquals count 0 }} disabled = "disabled" @{{/ifEquals}}>
