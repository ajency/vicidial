<!--?php print_r(json_encode($display_count));?-->
<script id="filter-brand-template" type="text/x-handlebars-template">
  <div class="kss_filter-list mt-1" data-filter="@{{template}}">
      <div class="filter-heading">
        <label class=" w-100 mb-0 pb-3 cursor-pointer @{{#if collapsed}} collapsed @{{/if}}" data-toggle="collapse" data-target="#collapseBrand" aria-expanded="false" aria-controls="collapseBrand">
         @{{filter_display_name}} <i class="fas fa-angle-up float-right"></i>
        </label>
      </div>
      <div id="collapseBrand" class="collapse@{{#if collapsed}}@{{else}} show @{{/if}}" data-field="@{{template}}">
...
<div id="filter-brand-template-content"></div>
@section('footjs-brand')
<script type="text/javascript" >
   var source   = document.getElementById("filter-brand-template").innerHTML;
   ...
   document.getElementById("filter-brand-template-content").innerHTML = html;
 </script>
 @stop