...
<div class="container sublanding-container pt-md-4">
   ...
   @foreach($static_elements['banner'] as $banner)
		<div static_element-id="{{$banner['sequence']}}" static_element-display_type="Section" static_element-type="{{$banner['type']}}" class="row mb-md-4 pb-md-4 kss-extension position-relative" page_slug="{{$gendername}}" >
			...
			<div class="col-md-5 d-flex">
				<div class="gender text-center d-flex align-items-center flex-column justify-content-between flex-1">
					...
					<p class="gender__years text-dark m-detach">{{$banner['element_data']['text']['text1']}}</p>				
						<a href="{{$banner['element_data']['image']['href']}}" class="btn kss-btn kss-btn--dark">{{$banner['element_data']['text']['text2']}}</a>
				</div>
			</div>
		</div>
	@endforeach	
