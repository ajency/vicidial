<div class="radio-wrap d-flex kss_sizes wo-image mb-4 single-radio">
    ...
    foreach ($params['variant_group']->{$selected_color_id}->variants as $size_set) {
	    ...
	    <label class="radio-label" for="size-{{$size_set->size->id}}" data-toggle="tooltip" data-placement="bottom" @php if($price_set['disabled']) { @endphp title="{{$size_set->size->name}}. Out of Stock" @php } else { @endphp title="{{$size_set->size->name}}" @php } @endphp>
