...
class AddressController extends Controller
{
    public function userAddAddress(Request $request)
    {
        ...
        if (isset($params['user'])) {
            ...
            $user->email_id = $params['user']['email'];
            $user->save();
        }
    ...
    {
        $request->validate(['default' => 'required', 'name' => 'required', 'phone' => 'required|digits:10', 'pincode' => 'required|digits:6', 'state_id' => 'required|numeric', 'address' => 'required', 'landmark' => 'present', 'city' => 'required', 'id' => 'required']);
        $params  = $request->all();
